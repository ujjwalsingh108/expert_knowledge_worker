...
def evaluate_answer(test: TestQuestion) -> tuple[AnswerEval, str, list]:
    ...
    judge_messages = [
        {
            "role": "system",
            "content": "You are an expert evaluator assessing the quality of answers. You must respond ONLY with a raw JSON object matching the requested schema.",
        },
        {
            "role": "user",
            ...
            {{
            "feedback": "string concise feedback comparing answers based on context",
            "accuracy": float (1 to 5),
            "completeness": float (1 to 5),
            "relevance": float (1 to 5)
            }}"""
        }
    ]

    # Call LLM without response_format, pointing directly to groq
    ...
    # Validate the raw JSON response text back into your Pydantic schema
